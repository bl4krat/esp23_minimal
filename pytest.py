...
def test_project(dut: IdfDut, log_minimum_free_heap_size: Callable[..., None]) -> None:
...
def test_project_linux(dut: IdfDut) -> None:
    dut.expect('Hello world!')
...
@idf_parametrize('target', ['linux'], indirect=['target'])
def test_project_macos(dut: IdfDut) -> None:
    dut.expect('Hello world!')
...
def test_project_host(app: QemuApp, dut: QemuDut) -> None:
